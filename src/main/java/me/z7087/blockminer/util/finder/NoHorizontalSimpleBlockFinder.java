...
public final class NoHorizontalSimpleBlockFinder {
    public static final class StructureFilterCache {
        private static final UniqueObjectCollection<BlockBreakStructure> STRUCTURES = UniqueObjectCollection.createInstance(
                "NoHorizontalSimpleBlockFinder.StructureFilterCache.BlockBreakStructure",
                BlockBreakStructure[]::new,
                BlockFinder.AllStructures
                        .stream()
                        .filter(structure -> {
                            if (structure.pistonOffset.getAxis() != Direction.Axis.Y
                                    || structure.pistonFace.getAxis() != Direction.Axis.Y)
                                return false;
        ...
        private static final UniqueObject<BlockPos>[] UNIQUE_POSITIONS = UniqueObjectPool.createUniqueObjectArray(UniqueObjectPool.withSInt32Limit("NoHorizontalSimpleBlockFinder.StructureFilterCache.BlockPos"), PositionsInSteps.S2.posList);
