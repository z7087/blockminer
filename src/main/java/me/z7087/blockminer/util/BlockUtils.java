...
public final class BlockUtils {
    ...
    public static ActionResult interactBlock(ClientPlayerInteractionManager interactionManager, ClientPlayerEntity player, ClientWorld world, Hand hand, BlockHitResult hitResult) {
        //#if MC >= 11900
    ...
    private static Vec3d getEyePos(PlayerEntity player) {
        ...
        //#else
        //$$ return new Vec3d(player.getX(), player.getY() + player.getEyeY(), player.getZ());
        //#endif
    }
    ...
    public static boolean playerCanTouchServerside(PlayerEntity player,
        ...
        return playerCanTouchServerside(player, blockPos, additionalRange, true, true, true);
    }

    public static boolean playerCanTouchServerside(PlayerEntity player,
                                                   BlockPos blockPos,
                                                   double additionalRange,
                                                   boolean isServerNewerThanOrEquals1_20_6,
                                                   boolean isServerNewerThanOrEquals1_19,
                                                   boolean isMine) {
        double blockPosX = blockPos.getX();
        double blockPosY = blockPos.getY();
        double blockPosZ = blockPos.getZ();
        // <和<=号是源码里写的 尽量不要混用
        if (isServerNewerThanOrEquals1_20_6) {
        ...
        } else if (isServerNewerThanOrEquals1_19) {
